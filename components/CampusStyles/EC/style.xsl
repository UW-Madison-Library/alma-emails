...
<xsl:stylesheet version="1.0" xmlns:xsl="https://www.w3.org/1999/XSL/Transform">
    <xsl:template name="libraryStyle">
		...
		<!-- CUSTOM STYLES -->
		<style type="text/css">		 
			.halfPage {min-height:1000px; height: 100%;  width: 50%;  position: fixed;  z-index: 1;  top: 0;  overflow-x: hidden; left: 0;}
			.halfPageMod {min-height: 1000px; position: absolute; top: 0; left: 0}
		</style>
		<!-- END CUSTOM STYLES -->	
   ...
   <xsl:template name="libraryLogo">
        <tr style="background-color: #000066;">
            <td>
                <a href="https://library.uwec.edu/" style="color: #005777;" target="_blank">
                    <img src="cid:logo.jpg" alt="Universitiy of Wisconsin - Eau Claire Logo" width="auto" height="100"/>
                </a>
            </td>
        </tr>
       <!--
                ...
                <table border="0" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                        <td align="center" valign="top" style="padding: 20px 0;">
                            <a href="https://library.uwec.edu/" style="color: #005777;" target="_blank">
                                <img src="https://www.wisconsin.edu/brand-style-guide/wp-content/uploads/sites/300/2023/10/UW-logo-color-e1725889687941-1024x273.png" width="200" height="134" alt="University of Wisconsin-Eau Claire" style="border: 0;"/>
                            </a>
                        </td>
                    </tr>
                </table>
                    ...
                    <xsl:for-each select="notification_data">
                        <xsl:choose>
                            <xsl:when test="receivers/receiver/user/preferred_first_name !=''">
                                Dear <xsl:call-template name="ProperCase"><xsl:with-param name="text"><xsl:value-of select="receivers/receiver/user/preferred_first_name"/></xsl:with-param></xsl:call-template>&#160;<xsl:call-template name="ProperCase"><xsl:with-param name="text"><xsl:value-of select="receivers/receiver/user/preferred_last_name"/></xsl:with-param></xsl:call-template>,
                            </xsl:when>
                            <xsl:otherwise>
                                Dear <xsl:call-template name="ProperCase"><xsl:with-param name="text"><xsl:value-of select="receivers/receiver/user/first_name"/></xsl:with-param></xsl:call-template>&#160;<xsl:call-template name="ProperCase"><xsl:with-param name="text"><xsl:value-of select="receivers/receiver/user/last_name"/></xsl:with-param></xsl:call-template>,
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:for-each>
                </td>
        ...
        <table cellspacing="0" cellpadding="0" border="0" role="presentation">
            <tr>
                <td style="padding-bottom: 40px; width: 500px; padding-top: 30px;">
                    ...
                    <p style="margin: 5px 0px 10px 0; padding-bottom: 10px; border-bottom: 2px solid #e5e5e5;">McIntyre Library</p>
                    <p style="padding-bottom: 5px;">For assistance, please:</p>
                    <table width="100%" cellspacing="0" cellpadding="0" border="0" role="presentation">
                        <tr>
                            <td style="padding-bottom: 10px; font-size: 16px;">                               
                                Email us at <a href="mailto:library@uwec.edu" target="_blank" style="color: #0073CE; text-decoration: underline;">library@uwec.edu</a>
                            </td>
                        </tr>
                    </table>
                </td>
    ...
    <xsl:template name="ILLSignature">
        <table cellspacing="0" cellpadding="0" border="0" role="presentation">
            <tr>
                <td style="padding-bottom: 40px; width: 500px; padding-top: 30px;">
                    <p style="padding: 0px 0;">Sincerely,</p>
                    <p style="fmargin: 5px 0px 10px 0; padding-bottom: 10px; border-bottom: 2px solid #e5e5e5;">McIntyre Library Interlibrary Loan (ILL)</p>
                    <p style="padding-bottom: 5px;">For assistance, please:</p>
                    <table width="100%" cellspacing="0" cellpadding="0" border="0" role="presentation">
                       <tr>
    ...
    <xsl:template name="AcquisitionsSignature">
        <table cellspacing="0" cellpadding="0" border="0" role="presentation">
            <tr>
                <td style="padding-bottom: 40px; width: 500px; padding-top: 30px;">
                    <p style="padding: 0px 0;">Sincerely,</p>
                    <p style="fmargin: 5px 0px 10px 0; padding-bottom: 10px; border-bottom: 2px solid #e5e5e5;">McIntyre Library</p>
                    <p style="padding-bottom: 5px;">For assistance, please:</p>
                    <table width="100%" cellspacing="0" cellpadding="0" border="0" role="presentation">
                       <tr>
                            <td style="padding-bottom: 10px;">
        ...
        <table role="presentation" class="wMobile" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px!important;">
            <tr>
                <td class="footer" align="center" valign="top" style="padding:20px 0;">
                    <p style="text-align: left; padding-left: 10px; margin: 0; font-size:14px!important;">&#169; University of Wisconsin -  Eau Claire - McIntyre Library</p>
                </td>
            </tr>
            <tr>
                <td class="footer" align="center" valign="top" style="padding:20px 0;">
                <xsl:for-each select="notification_data/general_data">
                    <p style="text-align: left; padding-left: 10px; margin: 0; font-size:14px!important;"><xsl:value-of select="letter_name"/> (<xsl:value-of select="current_date"/>)</p>
                </xsl:for-each>
                </td>	
            </tr>
        </table>
    ...
    <!-- PROPER CASE -->
    <!-- Code from: https://stackoverflow.com/questions/13122545/convert-first-character-of-each-word-to-upper-case/13122990 -->
    
    <!-- 
    The basic idea is that you call ProperCase, if it find a space, then it runs ProperCaseWord on everything before the space (i.e. the first word) and 
    then calls ProperCase again with the everything after the space (i.e. the rest of the sentence). Otherwise if no space is found (because it's got to 
    the last word in the sentence), then it just calls ProperCaseWord.
    
    The ProperCaseWord template simply translates the first character from lower to upper (if necessary) and all remaining characters from upper to lower (if necessary).
    -->
    <xsl:template name="ProperCase">
      <xsl:param name="text"/>
      <xsl:choose>
        <xsl:when test="contains($text,' ')">
          <xsl:call-template name="ProperCaseWord">
    ...
    <xsl:template name="ProperCaseWord">
      <xsl:param name="text"/>
    ...
    <!-- Example Usage 
    <xsl:call-template name="ProperCase">
       <xsl:with-param name="text">LNAME, FNAME MNAME</xsl:with-param>
    </xsl:call-template>
    -->
    <!-- END PROPER CASE -->
</xsl:stylesheet>